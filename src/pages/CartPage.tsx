...
export const CartPage: React.FC = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-3 sm:px-4 py-4 sm:py-8 space-y-6 sm:space-y-8">
      <h1 className="text-xl sm:text-3xl font-bold font-serif text-stone-900">
        Alışveriş Sepeti ({cartCount} Ürün)
      </h1>

      {/* Free Shipping Progress Indicator */}
      <div className="p-3.5 sm:p-4 rounded-2xl bg-amber-50 border border-amber-200/80 space-y-2">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1 text-xs font-bold text-amber-950">
          <div className="flex items-center gap-2">
            <Truck className="w-4 h-4 text-amber-700 shrink-0" />
            {remainingForFreeShipping === 0 ? (
              <span className="text-emerald-700 font-extrabold flex items-center gap-1">
                <Check size={14} className="shrink-0" /> Tebrikler! Ücretsiz kargo hakkı kazandınız.
              </span>
            ) : (
          ...
          <span className="self-end sm:self-auto text-[11px] text-amber-800">{freeShippingProgress.toFixed(0)}%</span>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 items-start">
        ...
        <div className="lg:col-span-8 space-y-4">
          <div className="bg-white rounded-2xl sm:rounded-3xl border border-stone-200 overflow-hidden divide-y divide-stone-100 shadow-sm">
            {cart.map(({ product, qty }) => {
              ...
              return (
                <div
                  key={product.id}
                  className="p-3 sm:p-6 hover:bg-stone-50/50 transition-colors"
                >
                  ...
                  {/* Mobile Mobile Compact Layout (< sm) */}
                  <div className="flex sm:hidden flex-col gap-3">
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0 pr-1">
                        <div className="text-[9px] font-bold uppercase text-amber-800">
                          {categoryName} • {product.brand}
                        </div>
                      ...
                      <button
                        onClick={() => removeFromCart(product.id)}
                        className="p-1.5 text-stone-400 hover:text-rose-600 rounded-lg hover:bg-rose-50 shrink-0"
                        title="Ürünü Sil"
                      >
                        <Trash2 size={16} />
                      </button>
                    ...
                    <div className="flex items-center justify-between pt-2 border-t border-stone-100">
                      ...
                      <div className="flex items-center border border-stone-200 rounded-lg overflow-hidden bg-stone-50">
                        ...
                        <span className="px-2 py-1 text-xs font-bold text-stone-900 min-w-[1.75rem] text-center">
                          {qty}
                        </span>
                        <button
                          onClick={() => updateCartQty(product.id, qty + 1)}
                      ...
                      <div className="text-right">
                        <span className="text-[10px] text-stone-400 block font-medium">Toplam</span>
        ...
        <div className="lg:col-span-4 space-y-6">
          <div className="bg-white rounded-2xl sm:rounded-3xl p-4 sm:p-6 border border-stone-200 shadow-sm space-y-5 lg:sticky lg:top-24">
            <h2 className="font-bold text-stone-900 text-base sm:text-lg font-serif pb-3 border-b border-stone-100">
              Sipariş Özeti
            </h2>
            ...
            <div className="space-y-2">
              ...
              {activeCoupon ? (
                ...
                <form onSubmit={handleApplyCouponSubmit} className="flex gap-2">
                  <input
                    ...
                    className="flex-1 min-w-0 px-3 py-2 rounded-xl bg-stone-50 border border-stone-200 text-xs focus:outline-none focus:border-amber-600 uppercase font-semibold"
                  />
                  <button
                    type="submit"
                    className="px-3.5 py-2 bg-stone-900 text-white font-bold text-xs rounded-xl hover:bg-amber-700 transition-colors shrink-0"
                  >
                    Uygula
                  </button>
                </form>
              ...
              <p className="text-[10px] text-stone-400">
                Kuponlar: <strong>MOBIDOLAP10</strong>, <strong>HOSGELDIN50</strong>,{' '}
            ...
            <div className="space-y-2.5 text-xs border-t border-stone-100 pt-4">
              ...
              <div className="pt-3 border-t border-stone-200 flex justify-between items-baseline">
                ...
                <span className="text-xl sm:text-2xl font-black text-amber-800 font-serif">
            ...
            <a
              href="#/odeme"
              className="w-full py-3.5 bg-stone-900 hover:bg-amber-700 text-white font-bold text-xs sm:text-sm rounded-xl flex items-center justify-center gap-2 shadow-lg transition-all"
            >
